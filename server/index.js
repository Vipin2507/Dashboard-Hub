...
const DEAL_STATUS_VALUES = new Set(["Hot", "Cold", "Active", "Pending", "Closed/Won", "Closed/Lost"]);

function resolveDealStatusForBulk(dealStatus, invoiceStatus) {
  const raw = String(dealStatus || "").trim();
  if (raw === "Closed/Won" || raw === "Closed/Lost") return raw;
  const lower = raw.toLowerCase();
  if (lower === "won" || lower === "closed won" || lower === "closed/won") return "Closed/Won";
  if (lower === "lost" || lower === "closed lost" || lower === "closed/lost") return "Closed/Lost";
  const inv = String(invoiceStatus || "")
    .trim()
    .toLowerCase();
  if (inv === "paid" || inv === "won" || inv === "closed won" || inv === "closed/won") return "Closed/Won";
  if (inv === "lost" || inv === "closed lost" || inv === "closed/lost") return "Closed/Lost";
  if (DEAL_STATUS_VALUES.has(raw)) return raw;
  return "Active";
}

/**
 * Bulk-update dealStatus for selected deals and/or deals matching fromStatus.
 * Body: { toStatus, fromStatus?, dealIds?, lossReason?, actorRole, actorUserId, actorTeamId, actorRegionId, changedByUserId, changedByName }
 */
app.post("/api/deals/bulk-status", (req, res) => {
  const body = req.body || {};
  const {
    actorRole,
    actorUserId,
  ...
  // Status changes require edit (sales_rep is stage-only).
  if (!canDealAction(normRole, "edit")) {
    return res.status(403).json({ error: "Forbidden" });
  }

  const nextStatus = String(toStatus || "").trim();
  if (!DEAL_STATUS_VALUES.has(nextStatus)) {
    return res.status(400).json({ error: "Valid toStatus is required" });
  }
  if (nextStatus === "Closed/Lost" && !isSuperAdminRole(normRole)) {
    return res.status(403).json({ error: "Only Super Admin can set Closed/Lost" });
  }
  if (nextStatus === "Closed/Lost" && (!lossReason || !String(lossReason).trim())) {
    return res.status(400).json({ error: "lossReason is required when status is Closed/Lost" });
  }

  const fromNormalized = fromStatus ? resolveDealStatusForBulk(fromStatus, null) : null;
  // If fromStatus was a canonical label, keep it; resolveDealStatusForBulk(Active) stays Active.
  const fromKey = fromStatus && DEAL_STATUS_VALUES.has(String(fromStatus).trim())
    ? String(fromStatus).trim()
    : fromNormalized;
  const idSet =
    Array.isArray(dealIds) && dealIds.length > 0
      ? new Set(dealIds.map((id) => String(id)))
  ...
  if (!fromKey && !idSet) {
    return res.status(400).json({ error: "Provide fromStatus and/or dealIds" });
  }
  ...
  if (fromKey) {
    rows = rows.filter(
      (d) => resolveDealStatusForBulk(d.dealStatus, d.invoiceStatus) === fromKey,
    );
  }
  ...
  const now = new Date().toISOString();
  const mergedLoss = nextStatus === "Closed/Lost" ? String(lossReason).trim() : null;
  const updateStmt = db.prepare(`
    UPDATE deals
  ...
  const tx = db.transaction((list) => {
    for (const d of list) {
      const prev = resolveDealStatusForBulk(d.dealStatus, d.invoiceStatus);
      const prevRaw = String(d.dealStatus || "Active");
      if (prev === nextStatus && prevRaw === nextStatus) {
        skippedUnchanged += 1;
        continue;
      }
      updateStmt.run({
      ...
      if (prev !== nextStatus) {
        logDealAudit(
          ...
          "deal_status_changed",
          {
            from: prevRaw,
            to: nextStatus,
            ...(mergedLoss ? { lossReason: mergedLoss } : {}),
            bulk: true,
          },
          changedByUserId,
          changedByName,
      ...
      logDealAudit(
        ...
        d.id,
        "deal_field_changed",
        { field: "dealStatus", oldValue: prevRaw, newValue: nextStatus, bulk: true },
        changedByUserId,
        changedByName,
      );
      const out = db.prepare("SELECT * FROM deals WHERE id = ?").get(d.id);
  ...
  tx(eligible);
  if (updated.length > 0) {
    broadcast({ type: "change", entity: "deals", action: "bulk_status_updated", count: updated.length });
  }
  res.json({
    updated: updated.length,
    skippedLocked,
